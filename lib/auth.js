...
// Terjemahkan pesan error Firebase ke bahasa yang lebih jelas
const friendly = e => {
  const raw = e.message
  if (raw.includes('OUT_OF_BAND_CODE') || raw.includes('INVALID_OOB_CODE'))
    return 'kode sudah dipakai / kedaluwarsa — kirim link baru lewat menu 1'
  if (raw.includes('EMAIL_NOT_FOUND')) return 'email tidak terdaftar di server'
  if (raw.includes('TOO_MANY_ATTEMPTS')) return 'terlalu banyak percobaan — tunggu sebentar lalu coba lagi'
  if (raw.includes('INVALID_EMAIL')) return 'format email tidak valid'
  if (raw.includes('MISSING_EMAIL')) return 'email tidak boleh kosong'
  if (raw.includes('USER_DISABLED')) return 'akun ini dinonaktifkan'
  if (raw.includes('OPERATION_NOT_ALLOWED')) return 'operasi tidak diizinkan untuk email ini'
  if (raw.includes('TOKEN_EXPIRED')) return 'token kedaluwarsa — login ulang lewat menu 1 & 2'
  if (raw.includes('INVALID_REFRESH_TOKEN')) return 'refresh token tidak valid — hapus sesi, lalu login ulang'
  if (raw.includes('USER_NOT_FOUND')) return 'akun tidak ditemukan di server'
  return raw
}
...
async function link(email) {
  try {
    ...
    return { ok: false, why: friendly(e) }
  }
}
